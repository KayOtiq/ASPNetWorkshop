...
namespace MyShuttle.Data
{
	public static class MyShuttleDataInitializer
	{
		...
		private static async Task CreateDefaultUser(IServiceProvider serviceProvider)
		{
			...
			var login = string.IsNullOrEmpty(configuration["DefaultUsername"]) ? "igorsych" : configuration["DefaultUsername"];
			var password = string.IsNullOrEmpty(configuration["DefaultPassword"]) ? "P@ssw0rd" : configuration["DefaultPassword"];
